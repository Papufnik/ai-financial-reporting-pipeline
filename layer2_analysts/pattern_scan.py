"""
pattern_scan.py -- shared "dot-connector" utility, importable by any Senior
Manager synthesis script in this pipeline.

(This is one module in a larger sanitized demo of a 4-layer AI-orchestrated
reporting pipeline built for a real small retail business. See the repo
README for the full architecture. This module in particular is already
completely generic -- no changes were needed to include it here verbatim.)

Extracted into its own module so the correlation/lag/guardrail logic has
ONE implementation, not one copy per role that could quietly drift apart.
Meant to be imported by each Senior Manager's own synthesis script and run
against whatever time-series data exists in that manager's domain -- NOT a
single centralized script that owns all the cross-referencing.
...
sample size, same-domain vs cross-domain tagging). Per this pipeline's
"surface facts, humans decide" model, a caller may apply its OWN
domain-specific filtering on top (e.g. deprioritizing anything trivially
explained by revenue scale) -- that judgment belongs in the caller, not
buried in this shared utility.
...
def scan_correlations(series, labels=None, min_overlap=10, lags=(0, 1, 2), domain_fn=None):
    ...
    series: dict[key] -> list of values (or None), all lists the SAME length
            and index-aligned to the same time periods (e.g. same N months
            in the same order).
    ...
    domain_fn: optional function(key) -> group label, used to tag
            ...
            first "-" in the key (e.g. "LAB-WAGES" -> domain "LAB").
