...
from validate_narrative_numbers import collect_allowed_numbers_from_data, check_narrative

"""
ceo_weekly_memo.py -- the CEO capstone in the C-Suite layer (Layer 4, see
repo README). Reads both Senior Managers' output (cfo_synthesis.json,
coo_synthesis.json), picks the handful of things that actually matter this
week, and writes them up as a short, plain-language memo. The idea: nobody
opens a dashboard they didn't ask for, but most business owners will read
one page a week if it's actually written for them.

Design principles:
  - This reads each Senior Manager's already-filtered `needsAttention` list
    rather than re-deriving priority from their raw category sections --
    the CFO and COO already did the "does this cross a threshold" judgment
    call; re-doing it here would just be a second, possibly-inconsistent
    opinion on the same facts.
  - Per "templated, not LLM-authored": every sentence in the structured
    "Worth a Look" section is an f-string built from real fields already
    present in the two Senior Manager JSONs -- nothing there is generated
    or paraphrased by a model. If a field is missing, the sentence
    referencing it is simply omitted (fail honest, don't fabricate).
  - Per "surface facts, humans decide": this memo states what's true and
    why it might matter, and explicitly does NOT issue directives. It
    closes by handing the decision back to the reader.
  - Output is both a structured JSON (for any future automation) and a
    plain-text memo (the actual reader-facing artifact).

ONE bounded, deliberate exception to "templated, not LLM-authored": an
optional narrative layer, off by default, that only ever runs if an
ANTHROPIC_API_KEY is present:
  ...
  - The prompt requires interpretive/causal claims to be marked inline as
    "[READ, NOT FACT: ...]" so a reader can always tell a computed fact
    from a read on it.
  - On ANY failure -- no ANTHROPIC_API_KEY set, API error, rate limit,
    failed number validation -- this falls back to the plain "Big Picture"
    framing that already existed, silently and safely. The structured
    "Worth a Look" section is NEVER touched by any of this; it's built the
    same deterministic way either way. The JSON output honestly records
    whether narrative actually ran this time (narrativeGenerated) or fell
    back (narrativeFallbackReason), never claims success it didn't earn.
...
def run_ceo_memo():
    ...
    cfo = load_json(os.path.join(data_dir, "cfo_synthesis.json"))
    coo = load_json(os.path.join(data_dir, "coo_synthesis.json"))
    ...
    if not cfo or not coo:
        print("[ERROR] Missing CFO or COO synthesis -- cannot write a CEO memo without both. "
              "Run cfo_synthesis.py and coo_synthesis.py first.")
        return None
    ...
    ttm_revenue = cash_forecast.get("ttmRevenue")
    framing_lines = []
    if growth_pct is not None and ttm_revenue is not None:
        direction = "up" if growth_pct >= 0 else "down"
        framing_lines.append(
            f"Trailing-12-month revenue is ${ttm_revenue:,.0f}, {direction} {abs(growth_pct)*100:.1f}% "
    ...
    # -------------------------------------------------------------------
    # One honest positive note, if the data actually supports one.
    # -------------------------------------------------------------------
    all_optimal = [
        ...
        dict(i, source="COO") for i in (
            coo.get("inventoryAndReorder", []) + coo.get("pricingHealth", []) + coo.get("staffingAndTraffic", [])
        )
        if i.get("severity") == "OPTIMAL"
    ]
    ...
    # -------------------------------------------------------------------
    # Optional narrative step -- see header comment. Falls back to None
    # (silently, safely) on any failure. allowed_numbers is built from the
    # raw source JSONs, not just the trimmed watch_items/framing_lines, so
    # a narrative referencing a real number that didn't make the top-5
    # watch list still validates correctly.
    # -------------------------------------------------------------------
    allowed_numbers = collect_allowed_numbers_from_data(cfo, coo, cash_forecast)
    ...
    # -------------------------------------------------------------------
    lines = []
    lines.append("# Weekly Business Memo")
    lines.append(f"*{memo_date}*")
    lines.append("")
    lines.append(
        "This is a plain-language pull from this week's numbers -- not a directive, just what's "
        "actually true right now and why it might matter. The owner is reviewing this before anything "
        "goes further."
    )
    lines.append("")
    if narrative:
    ...
    lines.append(
        ...
        f"cfo_synthesis.py and coo_synthesis.py's output. Reaches the owner only -- they decide what, "
        f"if anything, goes further.*"
    )
    memo_text = "\n".join(lines)
    ...
    json_out_file = os.path.join(data_dir, "ceo_weekly_memo.json")
    with open(json_out_file, "w", encoding="utf-8") as f:
        json.dump(memo_payload, f, indent=2)

    md_out_file = os.path.join(data_dir, "ceo_weekly_memo.md")
